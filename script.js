/* =============================================
   ...
   SHUTO EXPRESSWAY FULL ROUTE EDITION
   ============================================= */
'use strict';
...
function buildSpline(){
  const pts=RAW_ROUTE.map(([x,z])=>new THREE.Vector3(x,0,z));
...
const matConcB   =new THREE.MeshLambertMaterial({color:0x848494});
const matConcC   =new THREE.MeshLambertMaterial({color:0x606070});
const matMetal   =new THREE.MeshPhongMaterial({color:0x8899aa,shininess:50});
...
const matYellow  =new THREE.MeshBasicMaterial({color:0xffdd00});
...
function buildCityBackdrop(){
  ...
  for(let i=0;i<100;i++){
    ...
    build.position.copy(bpos); scene.add(build);
    ...
    if(i%7===0&&r(i*3+5)>0.55){
      ...
      chim.position.set(bpos.x+bw/4,bpos.y*0.7,bpos.z+2); scene.add(chim);
      const warn=new THREE.Mesh(new THREE.SphereGeometry(0.22,8,6),
        new THREE.MeshBasicMaterial({color:0xff2200}));
      warn.position.set(bpos.x+bw/4,bh*0.6+8,bpos.z+2); scene.add(warn);
    }
  }
}
...
function buildPlayer(){
  const sk=new THREE.MeshLambertMaterial({color:0xd4926a});
...
function drawMinimap(){
  ...
  const scale=Math.min((W-margin*2)/rangeX,(H-margin*2)/rangeZ);
  ...
  [{t:0.12,n:'大井JCT'},{t:0.28,n:'羽田'},{t:0.42,n:'C1南'},{t:0.60,n:'C2'},{t:0.76,n:'東雲'},{t:0.90,n:'辰巳JCT'}].forEach(j=>{
    ...
    minimapCtx.fillStyle='rgba(20,160,80,0.9)';minimapCtx.beginPath();minimapCtx.arc(jx,jz,3,0,Math.PI*2);minimapCtx.fill();
    minimapCtx.fillStyle='rgba(255,255,255,0.65)';minimapCtx.font='bold 6px sans-serif';minimapCtx.textAlign='center';
...
window.selectGfx=function(mode){
  gfxMode=mode;
  ['flat','normal','high'].forEach(m=>{
    document.getElementById('btn-gfx-'+m).classList.toggle('active',m===mode);
  });
...
function startGame(){
  ...
  phys.position.set(sp.x,0,sp.z);phys.yaw=0;phys.speed=0;phys.steer=0;phys.routeT=0;
  walkState.x=sp.x;walkState.z=sp.z;walkState.yaw=0;
...
function init(){
  ...
  function doStep(){
    if(i>=steps.length){
      ...
      setTimeout(()=>{const ls=document.getElementById('loading-screen');ls.style.opacity='0';setTimeout(()=>{ls.remove();document.getElementById('title-screen').classList.remove('hidden');},600);},200);
      return;
    }
    const p=Math.round((i/steps.length)*100);bar.style.width=p+'%';pct.textContent=p+'%';
    steps[i][1]();i++;setTimeout(doStep,90);
  }
  doStep();
